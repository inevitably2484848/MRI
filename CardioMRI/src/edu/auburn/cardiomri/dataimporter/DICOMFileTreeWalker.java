 ...
 * any contained DICOM files, create DICOMImage objects of those files, and 
 * fill a Study object with those DICOMImage objects.
 *  
...
public class DICOMFileTreeWalker extends SimpleFileVisitor<Path> {
	...
	/**
	 * @return		The ArrayList of DICOMImage objects found in a
	 * 				filetree walk.
	 */
	...
	public FileVisitResult visitFile(Path file, BasicFileAttributes attr)
		...
		if (attr.isRegularFile()) {
			...
			if (new String(cbuf).equals("DICM")) {
				...
				DICOMImage image = DICOM3Importer.makeDICOMImageFromFile(f.getPath());
				if (image == null) {
	...
	/**
	 * This method walks through a given filetree and returns the
	 * ArrayList of DICOMImage objects created from that filetree.
	 * 
	 * @param pathIn		The Path object representing the start of the
	 * 						filetree to be searched
	 * @return				An ArrayList of DICOMImage objects created from
	 * 						the walk over the filetree at pathIn
	 */
	 ...
	 * @param pathIn		The Path object representing the start of the
	 * 						filetree to be searched
	 * @param studyIn		The Study object that any DICOMImage objects should
	 * 						be added to. Can be empty or already filled.
	 * @return				The Study object with the newly added DICOMImage
	 * 						objects.
	 */
	public Study addFileTreeToStudy(Path pathIn, Study studyIn) {
		...
		for (DICOMImage image : dicomImageList) {
			try {
				...
				String error_entry = "[" + dateString + "] : Error occurred while processing SOPInstanceUID: " + ex.getMessage() + "\n"
						+ "DICOMImage's StudyID does not match Study's StudyID.\n";
				File error_log = new File(ERROR_LOG_FILENAME);
				try {
					Writer output = new BufferedWriter(new FileWriter(error_log, true));
