...
public class Slice implements Serializable {
	...
	/**
	 * This method adds a DICOMImage object to this Slice, by determining
	 * which Time object the DICOMImage belongs in, and if the Time object
	...
	public static class SliceComparator implements Comparator<Slice> {
		 ...
		 * 
		 * This method contains much of the logic for comparing and sorting 
		 * two Slice objects. While currently working well with Slices that 
		 * have the same eZ (the cross product of the two vectors in 
		...
		public int compare(Slice slice1, Slice slice2) {
			...
			Vector3d eY1 = new Vector3d(slice1.getImageOrientationPatient()[3],
					slice1.getImageOrientationPatient()[4],
					slice1.getImageOrientationPatient()[5]);
			Vector3d eY2 = new Vector3d(slice2.getImageOrientationPatient()[3],
			...
			/*if (Math.abs(eY1.getX() - eY2.getX()) < SLICE_COMPARISON_EPSILON
				...
				Vector3d framingVector = new Vector3d(1.0, 1.0, 1.0);
				Vector3d relationVector = eY1.cross(framingVector); // Vector
																	// perpendicular
																	// to eY1
																	// and
																	// framingVector

				double projection1 = slice1.getEz().dot(relationVector);
				double projection2 = slice2.getEz().dot(relationVector);
				
				if (projection1 - projection2 > SLICE_COMPARISON_EPSILON) return 1;
				if (projection1 - projection2 < -SLICE_COMPARISON_EPSILON) return -1;
				return 0;
			}*/
