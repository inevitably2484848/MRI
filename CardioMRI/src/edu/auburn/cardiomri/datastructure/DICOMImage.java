...
/**
 * This class represents a specific instance of an image imported from a DICOM
 * file. The class contains all appropriate header information imported into
 * corresponding fields. The class will also contain analysis markers added by
 * researchers.
 * 
 * @author Eric Turner
 *
...
public class DICOMImage implements Serializable {
    private static final long serialVersionUID = -5042324357271768283L;
    ...
    // EndoContour
    // EpiContour
    // ApexLandmark, BaseLandmark, LandmarkARV, LandmarkIRV, LandmarkMS,
    // LandmarkAPEX
    ...
    // Fields added for support of SourceImage.constructSourceImage(DICOMImage
    // image)
    ...
    public DICOMImage(HashMap<String, String> header, short[] pixelData)
         ...
         * ImagePositionPatient, ImageOrientationPatient, and PixelSpacing) have
         * been set in the input HashMap. If any of this information is missing,
         * a MissingParameterException
         */
        // Missing SliceThickness information invalidates DICOMImage creation,
        // as per requirements
        if (header.get("SliceThickness") == null) {
            throw new MissingParameterException("SliceThickness");
        }

        // Missing ImagePositionPatient information invalidates DICOMImage
        // creation, as per requirements
        if (header.get("ImagePositionPatient") == null) {
            throw new MissingParameterException("ImagePositionPatient");
        }

        // Missing ImageOrientationPatient information invalidates DICOMImage
        // creation, as per requirements
        if (header.get("ImageOrientationPatient") == null) {
            throw new MissingParameterException("ImageOrientationPatient");
        }

        // Missing PixelSpacing information invalidates DICOMImage creation, as
        // per requirements
        ...
        /*
         * If TriggerTime information is not given, triggerTime should default
         * to the value 0.0
         */
        ...
        /*
         * Current implementation cannot use DICOM files without PixelData. This
         * will likely change after the inclusion of MR spectroscopy support
         */
        ...
        this.spacingBetweenSlices = getDoubleValue(header
                .get("SpacingBetweenSlices"));
        this.heartRate = getDoubleValue(header.get("HeartRate"));
        this.imagePositionPatient = this.parseImagePositionPatient(header
                .get("ImagePositionPatient"));
        this.imageOrientationPatient = this.parseImageOrientationPatient(header
        ...
        this.photometricInterpretation = header
        ...
        this.planarConfiguration = getIntValue(header
        ...
        this.pixelRepresentation = getIntValue(header
                .get("PixelRepresentation"));
        this.pixelPaddingRangeLimit = getIntValue(header
                .get("PixelPaddingRangeLimit"));
        this.pixelPaddingValue = getIntValue(header.get("PixelPaddingValue"));
        this.largestMonochromePixelValue = getIntValue(header
                .get("LargestMonochromePixelValue"));
        this.largestImagePixelValue = getIntValue(header
     ...
     * @return A double[3] containing the vector components of the position
     *         vector of the DICOM slice.
     ...
     * @return A double[6] containing the vector components of the two
     *         orientation vectors. The first 3 components represent the vector
     *         that points in the direction of a row of the image. The last 3
     *         components represent the vector that points in the direction of a
     *         column of the image.
     * @author Tony Bernhardt
     */
    private double[] parseImageOrientationPatient(String imageOrientationPatient) {
        // Value comes in the form
        // "UU.UUUUU\VV.VVVVV\WW.WWWWW\XX.XXXXX\YY.YYYYY\ZZ.ZZZZZ"
    ...
    public void setRegistrationRotationScanner(
            double[] registrationRotationScanner) {
    ...
    public void setLargestMonochromePixelValue(
            Integer largestMonochromePixelValue) {
    ...
    /**
     * This method takes in a String (typically the String contained as the
     * value from the HashMap used in the constructor of DICOMImage), and
     * outputs an Integer value (when possible). This method acts similar to
     * Integer.parseInt(String string), with the major difference being that
     * this method can receive a null value as input and output a null value in
     * response, whereas Integer.parseInt(String string) would throw a
     * NullPointerException. This method will also handle both decimal and
     * hexadecimal (when preceded by '0x') values. Both radices are common
     * within the DICOM format.
     * 
     * @param inputValue - A String representing a numerical value, or null
     * @return - The Integer represented by inputValue, or null in the case of
     *         null input
     * @author Tony Bernhardt
     */
    private Integer getIntValue(String inputValue) {
        if (inputValue == null) {
            ...
            return Integer.parseInt(
                    inputValue.subSequence(2, inputValue.length()).toString(),
                    16);
        } else {
            inputValue.trim();
     ...
     * This method takes in a String (typically the String contained as the
     * value from the HashMap used in the constructor of DICOMImage), and
     * outputs a Double value (when possible). This method acts similar to
     * Double.parseDouble(String string), with the major difference being that
     * this method can receive a null value as input and output a null value in
     * response, whereas Double.parseDouble(String string) would throw a
     * NullPointerException.
     * 
     * @param inputValue - A String representing a numerical value, or null
     * @return - The Double represented by inputValue, or null in the case of
     *         null input
     * @author Tony Bernhardt
     */
    ...
    /**
     * This is an Exception class representing that information needed by the
     * CardioMRI program was not provided to the constructor of DICOMImage.
     * 
     * @author Tony Bernhardt
     *
     */
    public class MissingParameterException extends Exception {
         ...
         * 
         * @param message - The String representing which DICOM tag/attribute
    ...
    public static class ImageComparator implements Comparator<DICOMImage> {
        ...
        public int compare(DICOMImage image1, DICOMImage image2) {
            return image1.getSopInstanceUID().compareTo(
                    image2.getSopInstanceUID());
        }
