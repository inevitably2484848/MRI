...
public class Group implements Serializable {
	...
	/**
	 * This method will add a DICOMImage object to this Group, placing it
	 * in its proper Slice, or creating a Slice if the proper Slice does not
	 * exist. 
	 * 
	 * The sorting done here is important for proper visualization of a DICOM study.
	 * The sorting can be one of the more complex aspects of this project, and while
	 * the current implementation sorts well for series in which the Slices are
	 * positioned one on top of the other, correct sorting has not been figured
	 * out for 360-degree views and 3-plane studies.
	 * 
	 * Sorting logic is partially contained here, as well as in Slice.SliceComparator.
	 *  
	...
	public void addImage(DICOMImage image) {
		...
		for (Slice slice : slices) {
			...
			if (Math.abs(imgStackUnitVector.getX()
					- slice.getEz().getX()) > Slice.SLICE_COMPARISON_EPSILON)
				continue;
			if (Math.abs(imgStackUnitVector.getY()
					- slice.getEz().getY()) > Slice.SLICE_COMPARISON_EPSILON)
				continue;
			if (Math.abs(imgStackUnitVector.getZ()
					- slice.getEz().getZ()) > Slice.SLICE_COMPARISON_EPSILON)
				continue;
