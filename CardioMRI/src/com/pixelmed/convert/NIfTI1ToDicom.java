...
public class NIfTI1ToDicom {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/convert/NIfTI1ToDicom.java,v 1.11 2014/03/27 14:54:37 dclunie Exp $";
	...
	public static AttributeList generateGeometryFunctionalGroupsFromNIfTI1Header(NIfTI1Header nifti,AttributeList list,int numberOfFrames) throws DicomException {
...

		{
			...
			if (nifti.sform != NIfTI1Header.CoordinateTransform.UNKNOWN) {
				...
				// "Method 1" ... translation only ... theoretically "arbitrary" space, so just assume axial :(
System.err.println("Warning: no quaterinion or affine transform to Patient Coordinate System specified ... assuming axial images and translation only");
		...
		
		{
			...
			else {	// provide offsets even if (nifti.qform != NIfTI1Header.CoordinateTransform.UNKNOWN), since may be Method 1 (translation only)
	...
	public static AttributeList generateDICOMPixelDataModuleAttributesFromNIfTI1File(File inputFile,NIfTI1Header nifti,AttributeList list) throws IOException, DicomException, NIfTI1Exception {
		...
		switch(nifti.datatype) {
			...
			case FLOAT32:	aPixelData = new OtherFloatAttribute(PrivatePixelData.tagForFloatPixelData);
							sendPixelRepresentation = PrivatePixelData.sendPixelRepresentationForFloatOrDoublePixelData;
							pixelRepresentation = PrivatePixelData.pixelRepresentationForFloatPixelData;
							depth = 32;
							sendBitsStored = PrivatePixelData.sendBitsStoredAndHighBitForFloatOrDoublePixelData;
							sendHighBit = PrivatePixelData.sendBitsStoredAndHighBitForFloatOrDoublePixelData;
							samplesPerPixel = 1;
							photometricInterpretation = "MONOCHROME2";
							break;
			case COMPLEX64:	throw new DicomException("Conversion of "+nifti.datatype+" not supported");
			case FLOAT64:	aPixelData = new OtherDoubleAttribute(PrivatePixelData.tagForDoublePixelData);
							sendPixelRepresentation = PrivatePixelData.sendPixelRepresentationForFloatOrDoublePixelData;
							pixelRepresentation = PrivatePixelData.pixelRepresentationForDoublePixelData;
							depth = 64;
							sendBitsStored = PrivatePixelData.sendBitsStoredAndHighBitForFloatOrDoublePixelData;
							sendHighBit = PrivatePixelData.sendBitsStoredAndHighBitForFloatOrDoublePixelData;
							samplesPerPixel = 1;
		...
		PrivatePixelData.replacePixelData(list,aPixelData);
