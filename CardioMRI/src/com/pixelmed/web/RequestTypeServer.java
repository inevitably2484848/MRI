...
public class RequestTypeServer extends HttpServer {
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/web/RequestTypeServer.java,v 1.18 2013/01/26 15:14:10 dclunie Exp $";
...
if (webServerDebugLevel > 0) System.err.println("Received: "+dicomFileName+" from "+callingAETitle+" in "+transferSyntax);
	...
	public RequestTypeServer(String dataBaseFileName,String savedImagesFolderName,int dicomPort,String calledAETitle,int storageSCPDebugLevel,
			int wadoPort,int webServerDebugLevel,String rootURL,String stylesheetPath,String requestTypeToUseForInstances,int numberOfWorkers) {
	...
	public RequestTypeServer(String dataBaseFileName,String savedImagesFolderName,int dicomPort,String calledAETitle,int storageSCPDebugLevel,
		...
		this(dataBaseFileName,savedImagesFolderName,dicomPort,calledAETitle,storageSCPDebugLevel,wadoPort,webServerDebugLevel,rootURL,stylesheetPath,requestTypeToUseForInstances,defaultNumberOfWorkers);
	}
...
if (webServerDebugLevel > 1) System.err.println("RequestTypeServer.doCommonConstructorStuff(): rootURL supplied = "+rootURL);
		if (rootURL == null || rootURL.length() == 0) {
			try {
				InetAddress ineta = InetAddress.getLocalHost();
				if (ineta != null) {
					rootURL = ineta.getCanonicalHostName();
if (webServerDebugLevel > 1) System.err.println("RequestTypeServer.doCommonConstructorStuff(): rootURL from local InetAddress = "+rootURL);
				}
			}
