...
public class SynchronizeFromRemoteSCP {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/apps/SynchronizeFromRemoteSCP.java,v 1.13 2013/02/01 13:53:20 dclunie Exp $";
			...
			if (n > 0) {
				for (int i=0; i<n; ++i) {
					...
					if (child != null) {	// should not happen, but has been observed ... do not allow it to derail entire sync :(
						walkTreeDownToInstanceLevelAndRetrieve(child,uniqueKeys,retrieveDone);
					}
				}
			}
			else {
...
if (verbosityLevel > 2) System.err.println("Received: "+dicomFileName+" with SOPInstanceUID "+sopInstanceUID);
	...
	public SynchronizeFromRemoteSCP(DatabaseInformationModel databaseInformationModel,File savedInstancesFolder,
				String remoteHost,int remotePort,String remoteAE,int localPort,String localAE,boolean useGet,boolean queryAll,int verbosityLevel,int debugLevel,boolean anyTransferSyntax)
		...
		if (useGet) {
			...
			new Thread(new StorageSOPClassSCPDispatcher(localPort,localAE,savedInstancesFolder,StoredFilePathStrategy.BYSOPINSTANCEUIDHASHSUBFOLDERS,receivedObjectHandler,
														...
														false/*secureTransport*/,debugLevel)).start();
		}
...
if (verbosityLevel > 0) {
	 ...
	 *
	 * @param	arg		array of 7, 8, 9, 10 or 11 strings - the fully qualified path of the database file prefix, the fully qualified path of the saved incoming files folder,
	...
	public static void main(String arg[]) {
		try {
			if (arg.length >= 7 && arg.length <= 12) {
				...
				boolean anyTransferSyntax = arg.length > 11 ? (arg[11].trim().toUpperCase(java.util.Locale.US).equals("ANY") ? true : false) : false;
				
				if (useGet && anyTransferSyntax) {
					System.err.println("ANY Transfer Syntax can only be used with MOVE not GET");
					System.exit(0);
				}
				...
				new SynchronizeFromRemoteSCP(databaseInformationModel,savedInstancesFolder,remoteHost,remotePort,remoteAE,localPort,localAE,useGet,queryAll,verbosityLevel,debugLevel,anyTransferSyntax);
