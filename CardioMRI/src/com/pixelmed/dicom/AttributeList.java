...
public class AttributeList extends TreeMap<AttributeTag,Attribute> {
	...
	/***/
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/AttributeList.java,v 1.143 2014/08/16 13:24:54 dclunie Exp $";
	 ...
	 * @exception	DicomException	if the value length is not reasonable
	 */
		...
		if (vl > maximumShortVRValueLength && ValueRepresentation.isShortValueLengthVR(vr)) {
			if (!tag.equals(TagFromName.DVHData)
			 ...
			 && !tag.equals(TagFromName.HistogramData)
				...
				System.err.println("Fixed VL ("+vl+" dec, 0x"+Long.toHexString(vl)+") for "+tag+" - not encodable as explicit VR");
			}
		}
...
//System.err.println("readNewSequenceAttribute: Item byteOffset="+byteOffset);
					AttributeList list = new AttributeList();
					byteOffset=list.read(i,byteOffset,vl,false,specificCharacterSet,null/*stopAtTag*/,null/*ReadTerminationStrategy*/);
...
//System.err.println("AttributeList.readNewSequenceAttribute(): end");
	...
	//	return read(i,byteOffset,lengthToRead,stopAfterMetaInformationHeader,specificCharacterSet,null/*stopAtTag*/,null/*ReadTerminationStrategy*/);
	//}
	...
	private long read(DicomInputStream i,long byteOffset,long lengthToRead,boolean stopAfterMetaInformationHeader,
			SpecificCharacterSet specificCharacterSet,AttributeTag stopAtTag,ReadTerminationStrategy strategy) throws IOException, DicomException {
...
System.err.println("Ignoring bad Item at "+byteOffset+" "+tag+" VL=<0x"+Long.toHexString(vl)+">");
				...
				if (explicit) {
					...
					if (vr[0] == '-' && vr[1] == '-') {
						System.err.println("AttributeList.read(): "+tag+" encountered illegal Explicit VR of pair of hyphens in Explicit VR Transfer Syntax ... compensating");
						byteOffset+=2;
						vr[0]='U';
						vr[1]='N';
					}
					else if (vr[0] == 0 && vr[1] == 0) {
						System.err.println("AttributeList.read(): "+tag+" encountered illegal Explicit VR of pair of zero bytes in Explicit VR Transfer Syntax ... compensating");
						byteOffset+=2;
						vr[0]='U';
						vr[1]='N';
					}
...
System.err.println("AttributeList.read(): "+tag+" encountered incorrect short Value Length Form for UT or UN Value Representation in Explicit VR Transfer Syntax ... compensating");
						}
					}
				}
				else {
...
//System.err.println("AttributeList.read(): For tag "+tag+" overriding explicit VR "+ValueRepresentation.getAsString(vr)+" with "+ValueRepresentation.getAsString(vrd));
				...
				// this is dangerous, because contents will be IVRLE not EVRlE encoded :(
				
				if (ValueRepresentation.isUnknownVR(vr) && tag.isPrivate() && ClinicalTrialsAttributes.isSafePrivateSequenceAttribute(getPrivateCreatorString(tag),tag)) {
System.err.println("AttributeList.read(): "+tag+" explicit VR was UN but we know this is a safe private SQ");
					vr[0]='S';
					vr[1]='Q';
				}
...
//System.err.println("Undefined length encapsulated Pixel Data: TransferSyntax UID "+tsuid);
					...
					// when constructing EncapsulatedInputStream, take care whether or not to enable JPEG EOI detection to allow fragment spanning and adding removal
					EncapsulatedInputStream ei = new EncapsulatedInputStream(i,!tsuid.equals(TransferSyntax.PixelMedEncapsulatedRawLittleEndian) && !tsuid.equals(TransferSyntax.RLE));
...
//System.err.println("AttributeList.read(): Decompress Pixel Data");
						...
						else if (tsuid.equals(TransferSyntax.RLE)) {
//System.err.println("TransferSyntax.RLE");
							...
							if (bytesPerSample == 1) {
//System.err.println("bytesPerSample = 1");
								byte[] bytePixelData = new byte[wordsPerFrame*frames];
//System.err.println("Number of frames = "+frames);
//System.err.println("wordsPerFrame = "+wordsPerFrame);
//System.err.println("pixelsPerFrame = "+pixelsPerFrame);
								int frameOffset=0;
								//try {
								for (int f=0; f<frames; ++f) {
...
//System.err.println("NUmber of segments = "+numberofSegments);
									long[] segmentOffsets = new long[15];
									ei.readUnsigned32LittleEndian(segmentOffsets,0,15);
									for (int soi=0; soi<15; ++soi) {
//System.err.println("Segment ["+soi+"] offset = "+segmentOffsets[soi]);
...
//System.err.println("Doing sample = "+s);
											int currentOffset = ei.getOffsetOfNextByteToReadFromStartOfFragment();
//System.err.println("At fragment offset "+currentOffset);
...
//System.err.println("Skipped "+bytesToSkipToStartOfSegment+" to segment offset "+segmentOffsets[s]);
											}
...
//System.err.println("got = "+got+" pixels");
...
//System.err.println("Segment ["+soi+"] offset = "+segmentOffsets[soi]);
									...
									if (isColorByPlane) {
										...
										for (int s=0; s < samplesPerPixel; ++s) {
//System.err.println("Doing sample = "+s);
//System.err.println("Doing firstsegment");
...
//System.err.println("At fragment offset "+currentOffset);
												...
												if (bytesToSkipToStartOfSegment > 0) {
													ei.skipInsistently(bytesToSkipToStartOfSegment);
//System.err.println("Skipped "+bytesToSkipToStartOfSegment+" to segment offset "+segmentOffsets[segment]);
												}
												else if (bytesToSkipToStartOfSegment < 0) {
...
//System.err.println("got = "+got+" bytes for first segment");
//System.err.println("Doing secondsegment");
											++segment;
...
//System.err.println("got = "+got+" bytes for second segment");
											for (int p=0; p<pixelsPerFrame; ++p) {
...
//System.err.println("AttributeList.read(): Scanning for ImageIO plugin codecs");
							...
							pixelDataWasDecompressed = true;
							colorSpaceWasConvertedToRGBDuringDecompression = false;
							boolean colorSpaceWillBeConvertedToRGBDuringDecompression = false;		// do not set this generally ... be specific to each scheme (00704)
...
//System.err.println("Choosing reader from "+readerDescription+" "+readerVendorName+" "+readerVersion+" over Standard JPEG Image Reader");
									...
									if (reader != null) {
										if (tsuid.equals(TransferSyntax.JPEGExtended) && readerDescription.equals("Standard JPEG Image Reader") && (readerVendorName.equals("Sun Microsystems, Inc.") || readerVendorName.equals("Oracle Corporation"))) {
...
//System.err.println("AttributeList.read(): Do not decompress Pixel Data");
						...
						for (int f=0; f<frames; ++f) {
//System.err.println("AttributeList.read(): Doing compressed frame "+f);
...
//System.err.println("AttributeList.read(): bytesTotal = "+bytesTotal+" for frame "+f);
							byte[] frame = null;
							if (bytesTotal > 0) {
...
//System.err.println("AttributeList.read(): b length = "+l);
						...
						ei.readSequenceDelimiter();		// since we terminated loop on number of frames, rather than keeping going until ran out, we need to absorb the delimiter
						doneReadingEncapsulatedData=true;
						pixelDataWasDecompressed = false;
						pixelDataWasLossy=false;
						lossyMethod=null;
						compressionRatio=0;
						OtherByteAttributeMultipleCompressedFrames ac = new OtherByteAttributeMultipleCompressedFrames(tag,frameArray);
						a = ac;
					}
...
//System.err.println("Length attribute class="+a.getClass());
							...
							byteOffset=read(i,byteOffset,metaLength,false,null,stopAtTag,strategy);		// detects and sets transfer syntax for reading dataset
							i.setReadingDataSet();
...
//System.err.println("Calling read");
								byteOffset=read(i,byteOffset,0xffffffffl,false,null,stopAtTag,strategy);	// read to end (will detect and set own SpecificCharacterSet)
...
//System.err.println("AttributeList.read(): end");
	 ...
	 * @exception	DicomException	if an error in DICOM parsing
	 */
	public long readOnlyMetaInformationHeader(DicomInputStream i) throws IOException, DicomException {
		return read(i,i.getByteOffsetOfStartOfData(),0xffffffffl,true/*stopAfterMetaInformationHeader*/,null/*SpecificCharacterSet*/,null/*stopAtTag*/,null/*ReadTerminationStrategy*/);
...
//System.err.println("read(DicomInputStream i,AttributeTag stopAtTag="+stopAtTag+"):");
		return read(i,i.getByteOffsetOfStartOfData(),0xffffffffl,false/*stopAfterMetaInformationHeader*/,null/*SpecificCharacterSet*/,stopAtTag,null/*ReadTerminationStrategy*/);
	}
...
//System.err.println("read(DicomInputStream i,AttributeTag stopAtTag="+stopAtTag+"):");
		return read(i,i.getByteOffsetOfStartOfData(),0xffffffffl,false/*stopAfterMetaInformationHeader*/,null/*SpecificCharacterSet*/,stopAtTag,strategy);
	}
...
//System.err.println("read(DicomInputStream i,AttributeTag stopAtTag="+stopAtTag+"):");
		return read(i,i.getByteOffsetOfStartOfData(),0xffffffffl,false/*stopAfterMetaInformationHeader*/,null/*SpecificCharacterSet*/,null/*stopAtTag*/,strategy);
	}
	...
	public long read(DicomInputStream i) throws IOException, DicomException {
		return read(i,i.getByteOffsetOfStartOfData(),0xffffffffl,false/*stopAfterMetaInformationHeader*/,null/*SpecificCharacterSet*/,null/*stopAtTag*/,null/*ReadTerminationStrategy*/);
	}
	 ...
	 * @exception	IOException		if an I/O error occurs
	 * @exception	DicomException	if an error in DICOM encoding
	 */
...
//System.err.println("Writing "+a);
	 ...
	 * <p>Does nothing if the attribute was not already present.</p>
	 *
	 * @deprecated	See {@link #replaceWithValueIfPresent(AttributeTag,String) replaceWithValueIfPresent()}
	 *
	 * @param	tag			key (tag) for which the attribute should be replaced
...
//System.err.println("AttributeList.correctDecompressedImagePixelModule(): aLossyImageCompressionRatio= "+aLossyImageCompressionRatio);
	...
	/**
	 * <p>Get the corrected PhotometricInterpretation iff the color space was a compressed input transfer syntax that was decompressed during reading.</p>
	 *
	 * <p> E.g., from YBR_FULL_422 for JPEG lossy to RGB.</p>
	 *
	...
	public String getDecompressedPhotometricInterpretation(String vPhotometricInterpretation) {
		if (pixelDataWasDecompressed && colorSpaceWasConvertedToRGBDuringDecompression) {
	...
	public void correctDecompressedImagePixelModule() throws DicomException {
		if (pixelDataWasDecompressed && colorSpaceWasConvertedToRGBDuringDecompression && Attribute.getSingleIntegerValueOrDefault(this,TagFromName.SamplesPerPixel,0) > 1) {
			String vPhotometricInterpretation = Attribute.getSingleStringValueOrEmptyString(this,TagFromName.PhotometricInterpretation);
...
//System.err.println("findAllNestedReferencedSOPInstanceUIDs(): adding "+referencedSOPInstanceUID);
	 ...
	 * @exception	DicomException	if unable to make an Attribute
	 */
