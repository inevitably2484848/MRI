...
public class IntegerScalingOfFloatingPointPixelData {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/apps/IntegerScalingOfFloatingPointPixelData.java,v 1.12 2014/03/11 15:13:27 dclunie Exp $";
...
System.err.println("performIntegerScaling(): slope = "+slope+" (1/slope = "+(1/slope)+")");
			...
			
			{
				...
				PrivatePixelData.replacePixelData(list,dstPixelData);
	...
	public static void reverseIntegerScaling(String inputFileName,String outputFileName,boolean toDouble) throws DicomException, FileNotFoundException, IOException {
		...
		if (nPixels == 0) {
			...
			if (toDouble) {
				...
				dstPixelData = new OtherDoubleAttribute(PrivatePixelData.tagForDoublePixelData);
				dstPixelData.setValues(dstPixels);
				...
				dstPixelData = new OtherFloatAttribute(PrivatePixelData.tagForFloatPixelData);
				dstPixelData.setValues(dstPixels);
			...
			
			{
				...
				if (PrivatePixelData.sendBitsStoredAndHighBitForFloatOrDoublePixelData) {
				...
				if (PrivatePixelData.sendPixelRepresentationForFloatOrDoublePixelData) {
					Attribute a = new UnsignedShortAttribute(TagFromName.PixelRepresentation); a.addValue(0); list.put(a);
				}
				else {
