...
public class InsertRealWorldValueMap {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/apps/InsertRealWorldValueMap.java,v 1.3 2014/03/20 16:41:14 dclunie Exp $";
	
	private static final AttributeTag attributeTagForQuantityDefinitionCodeSequence = new AttributeTag(0x0041,0x1001);
	private static final AttributeTag attributeTagForQuantityDefinitionCodeSequencePrivateCreator = new AttributeTag(0x0041,0x0010);
	private static final String quantityDefinitionCodeSequencePrivateCreator = "PixelMed Publishing";
			...
			try {
				...
				// don't care about which SOP Class per se
				{
					...
					if (quantityDefinitionContentItems != null && !quantityDefinitionContentItems.isEmpty()) {
						SequenceAttribute aQuantityDefinitionCodeSequence = new SequenceAttribute(attributeTagForQuantityDefinitionCodeSequence);
						rwvmList.put(aQuantityDefinitionCodeSequence);
						if (attributeTagForQuantityDefinitionCodeSequencePrivateCreator != null) {
							Attribute a = new LongStringAttribute(attributeTagForQuantityDefinitionCodeSequencePrivateCreator); a.addValue(quantityDefinitionCodeSequencePrivateCreator); rwvmList.put(a);
						}
						for (ContentItem ci : quantityDefinitionContentItems) {
							aQuantityDefinitionCodeSequence.addItem(ci.getAttributeList());
...
System.err.println("extractContentItems(): starting arg.length = "+arg.length+", start = "+start+", end = "+end);
		List<ContentItem> listOfContentItems = new ArrayList<ContentItem>();
		while (end > start) {
			int remaining = end - start + 1;
System.err.println("extractContentItems(): remaining = "+remaining);
...
System.err.println("extractContentItems(): contentItem = "+contentItem);
			if (contentItem != null) {
