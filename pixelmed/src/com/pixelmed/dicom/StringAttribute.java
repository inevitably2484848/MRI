...
abstract public class StringAttribute extends Attribute {
	...
	/***/
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/dicom/StringAttribute.java,v 1.27 2014/02/21 14:29:11 dclunie Exp $";
...
//System.err.println("StringAttribute.isCharacterInValueValid(): c = "+c);
					...
					if (v != null && v.length() > 0) {
						...
						for (int j=0; j<v.length(); ++j) {
							...
							if (allowRepairOfInvalidCharacterReplacement() && !isCharacterInValueValid(cp)) {
								if (getInvalidCharacterReplacement() != 0) {
									buf.append(getInvalidCharacterReplacement());
								}
							}
							else {
